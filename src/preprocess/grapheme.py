...
def generate_grapheme_labels(transcripts: List[str], save_path: str):
    logger.info("Create grapheme labels started")
    label_dict = {}
    os.makedirs(save_path, exist_ok=True)

    for transcript in transcripts:
        grapheme_script = (
            " ".join(unicodedata.normalize("NFKD", transcript).replace(" ", "|"))
            .upper()
            .split()
        )
        for grapheme in grapheme_script:
            if grapheme not in label_dict.keys():
                label_dict[grapheme] = 1
...
def generate_grapheme_script(
    audio_paths: List[str], transcripts: List[str], save_path: str
) -> None:
    logger.info("Create grapheme script started")
