...
def load_label(filepath: str) -> Tuple[Dict[str, str], Dict[str, str]]:
    grpm2id = dict()
    id2grpm = dict()
...
def sentence_to_target(transcript: str, grpm2id: Dict[str, str]) -> str:
    target = str()
...
def sentence_to_grapheme(audio_paths: str, transcripts: str, vocab_dest: str = './data') -> None:
...
def generate_grapheme_labels(grapheme_transcripts: List[str], vocab_dest: str = './data') -> None:
    vocab_list = list()
    vocab_freq = list()
